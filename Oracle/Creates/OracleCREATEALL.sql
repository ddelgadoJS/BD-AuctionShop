...
BEGIN
    ...
    --Crea subasta.
    INSERT INTO SYS.SUBASTA (IDITEM, ALIASVENDEDOR, MODOENTREGA, PRECIOINICIAL, PRECIOACTUAL, FECHAHORAFIN, COMENTARIOVENDEDOR, ESTADOSUBASTA)
...
BEGIN
  ...
  IDSUBASTAr SYS.SUBASTA.ID%TYPE;
  ...
  BEGIN
    OPEN CSUBASTA;
      LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(IDSUBASTAr || ', ' || ALIASVENDEDORr || ', ' || TO_CHAR(FECHAHORAFINr,'DD-MON-YYYY') || ', ' || TO_CHAR(FECHAHORAFINr,'HH24:MI'));
      END LOOP;
    CLOSE CSUBASTA;
  END;
...
BEGIN
  DECLARE 
  CURSOR CSUBASTA IS 
    SELECT SUBASTA.ID, SUBASTA.FECHAHORAFIN, SUBASTA.IDITEM, SUBASTA.PRECIOINICIAL, SUBASTA.PRECIOACTUAL, SUBASTA.COMENTARIOCOMPRADOR, SUBASTA.ESTADOSUBASTA
    FROM SYS.SUBASTA SUBASTA
  ...
  BEGIN
    OPEN CSUBASTA;
      LOOP
        FETCH CSUBASTA INTO IDSUBASTAr, FECHAHORAFINr, IDITEMr, PRECIOINICIALr, PRECIOACTUALr, COMENTARIOCOMPRADORr, ESTADOSUBASTAr;
        EXIT WHEN CSUBASTA%NOTFOUND;
        DBMS_OUTPUT.PUT_LINE(IDSUBASTAr || ', ' || PRECIOINICIALr || ', ' || PRECIOACTUALr || ', ' || COMENTARIOCOMPRADORr || ', ' || FECHAHORAFINr || ', ' || ESTADOSUBASTAr);
      END LOOP;
    CLOSE CSUBASTA;
  END;
  COMMIT;
